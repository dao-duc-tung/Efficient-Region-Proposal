...
GenROI_Ver genROI = GenROI_Ver();
HOG hog = HOG();
string pIn, pOut, name, p;
...
void doROI_wHOG(){
	pOut = "C:/TUNGDD/Code/zexp/output_ROI_Ver_HOG_1/";
	...
	for (int i=4; i<file_list.size()-1; i+=4)
	{
		...
		// ROI + HOG_OpenCV
		genROI.doMainJob(p, p);
		hog.hogForScoredROIs(p, genROI.pROI, genROI.nROI);
