#include"GenROI_Proposal.h"

GenROI_Proposal::GenROI_Proposal() {
	pLabel = "C:/TUNGDD/Code/zexp/src_ETHZ/lp-annot.idl";
}

int GenROI_Proposal::THRES_GRAD = 20;
float GenROI_Proposal::THRES_NMS_ROI = 0.7;
int GenROI_Proposal::NUM_ROI = 0;

int GenROI_Proposal::MIN_WIDTH = 32, GenROI_Proposal::MIN_HEIGHT = 64;
int GenROI_Proposal::MAX_WIDTH = 120, GenROI_Proposal::MAX_HEIGHT = 240;

int GenROI_Proposal::STEP_WIDHT = 8, GenROI_Proposal::STEP_HEIGHT = 16;
int GenROI_Proposal::MIN_PIXEL_ROI = 15, GenROI_Proposal::MIN_PIXEL_LEG = 10;
int GenROI_Proposal::ROW_STEP_WINDOW = 8, GenROI_Proposal::COL_STEP_WINDOW = 8;

vector<std::pair<Rect, float>> GenROI_Proposal::doMainJob(string sI1, string sI2) {
...
Point GenROI_Proposal::getTransVector(Mat I1, Mat I2) {
	int rows = I1.rows, cols = I1.cols, depth = I1.depth();
	if (depth > 1) {
		cv::cvtColor(I1, I1, CV_BGR2GRAY);
		cv::cvtColor(I2, I2, CV_BGR2GRAY);
	}
	Mat I1G = getGrayCode(I1);
	Mat I2G = getGrayCode(I2);

	int dis_fr_bound = 5, sub_size = 20;
	int search_size = sub_size * 3;
	vector<Point> sub_win = initCorners(I1, dis_fr_bound, search_size);

	//estimate local motion
	int num_sub_win = sub_win.size();
	vector<Point> error_coor;
	int half_subW = sub_size / 2;

	for (int i = 0; i < num_sub_win; i++) {
		//get sub_win in frame 1
		int rr = sub_win[i].y, cc = sub_win[i].x;
		Mat subW = I1G(Range(rr - half_subW, rr + half_subW), Range(cc - half_subW, cc + half_subW));
		uchar* pSubW = (uchar*)(subW.data);

		//search matching-win in search-win in frame 2
		vector<std::pair<Point, int>> min_coor;
		min_coor.push_back(std::pair<Point, int>(Point(0, 0), 256 * sub_size*sub_size));

		for (int r = rr - search_size / 2 + half_subW; r < rr + search_size / 2 - half_subW; r++) {
			for (int c = cc - search_size / 2 + half_subW; c < cc + search_size / 2 - half_subW; c++) {
				Mat temp_subW = I2G(Range(r - half_subW, r + half_subW), Range(c - half_subW, c + half_subW));
				Mat error_tempW;
				bitwise_xor(subW, temp_subW, error_tempW);
				int error_temp = countNonZero(error_tempW);
				if (min_coor[0].second > error_temp) {
					min_coor[0].first.x = c;
					min_coor[0].first.y = r;
					min_coor[0].second = error_temp;
				}
			}
		}
		error_coor.push_back(min_coor[0].first);
	}

	//calc global motion
	Point delta(0, 0);
	for (int i = 0; i < error_coor.size(); i++) {
		delta.x += error_coor[i].x - sub_win[i].x;
		delta.y += error_coor[i].y - sub_win[i].y;
	}
	delta.x /= -num_sub_win; delta.y /= -num_sub_win;

	//draw for debug
	Mat tempI2 = I2.clone();
	for (int i = 0; i < sub_win.size(); i++) {
		Rect r(sub_win[i].x - 8, sub_win[i].y - 8, 16, 16);
		rectangle(tempI2, r.tl(), r.br(), Scalar(0, 255, 0), 1);
		r = Rect(error_coor[i].x - 8, error_coor[i].y - 8, 16, 16);
...
Mat GenROI_Proposal::getMovingArea(Mat I1, Mat I2, Point transVector) {
	...
	erode(MovingArea, MovingArea, getStructuringElement(MORPH_RECT, Size(2, 2)));
	dilate(MovingArea, MovingArea, getStructuringElement(MORPH_RECT, Size(3, 3)));

	return MovingArea;
}
void GenROI_Proposal::generateROI(Mat I2, Mat MovingArea, vector<std::pair<Rect, float>>& pROI, vector<std::pair<Rect, float>>& nROI) {
	...
	int WIDHT_ROI = GenROI_Proposal::MIN_WIDTH, HEIGHT_ROI = GenROI_Proposal::MIN_HEIGHT;
	...
	//perform NMS
	nms(ROI, GenROI_Proposal::THRES_NMS_ROI, filteredROI, removedROI);
...
void GenROI_Proposal::nms(vector<std::pair<Rect, float>> ROIs, float overlapThresh, vector<std::pair<Rect, float>>& filteredROI, vector<std::pair<Rect, float>>& removedROI) {
...
vector<std::pair<Rect, float>> GenROI_Proposal::evaluateROI_v2(string img, vector<std::pair<Rect, float>> pROI, vector<std::pair<Rect, float>> nROI, vector<Rect> pedCoor) {
	...
	for (int i = 0; i < pROI.size(); i++) {
		...
		if (true_roi) {
			...
			//trueROI.push_back(std::pair<Rect, bool>(pROI[i], true));
		}
		else {
			fp++;
			rectangle(ori, pROI[i].first, Scalar(100, 50, 225), 1);
			//trueROI.push_back(std::pair<Rect, bool>(pROI[i], false));
		}
	}
...
void GenROI_Proposal::displayRect(Mat I2, vector<Rect> ROIs) {
	for (int i = 0; i < ROIs.size(); i++) {
		Rect r = ROIs[i];
		rectangle(I2, r.tl(), r.br(), Scalar(0, 255, 0), 1);
	}
	GenROI_Proposal::outImg = I2;
}

Mat GenROI_Proposal::translateImage(Mat I1, Point transVector) {
...
vector<Point> GenROI_Proposal::initCorners(Mat I1, int dis_fr_bound, int search_size) {
	int rows = I1.rows, cols = I1.cols;
	int num_sub_win = 4;
...
Mat GenROI_Proposal::getGrayCode(Mat I) {
	int rows = I.rows, cols = I.cols;
	Mat out(rows, cols, I.type(), Scalar::all(0));
	uchar* pOut = (uchar*)(out.data);
...
int GenROI_Proposal::getVanishingLine(string pLabel) {
...
void GenROI_Proposal::gradient(Mat input, Mat& magnitude, Mat& vGrad, Mat& hGrad, int thres) {
	Mat h, v, m, a;
