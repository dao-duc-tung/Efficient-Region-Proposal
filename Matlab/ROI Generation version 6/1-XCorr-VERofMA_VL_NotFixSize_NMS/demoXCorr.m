...
pOut = 'E:\DAO DUC TUNG\TestBMS\output_ROI_MODE_1';
...
while(num_running < size(SCORE_THRESH,1))
    ...
    for t = 5:4:(numel(file_list))-105
        ...
        %% evaluate ROI
        [evalImg,tp,fp,tn,fn,num_ped] = evaluateROI_v2(pIn,pLabel,name2,pROI,nROI);
...
%         imwrite(MovingArea,fullfile(pOut,strcat(name2(1:end-4),'-Move',name2(end-3:end))));
    ...
    if first_run == 1
        ...
        for k=1:NUM_PART_SCORE_THRESH-8
